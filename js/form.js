...
(function () {
  ...
  window.form = {
    advertFieldsStateSwitcher() {
      window.util.fieldsStateSwitcher(advertFields);
    },
    addressFieldAssigner: (mainPinYOffset) => {
      advertAddressField.value = window.util.addressValueGetter(mainPinYOffset);
    },
    formActivator: () => {
      window.form.addressFieldAssigner(84);
      window.util.classRemover(advertForm, `ad-form--disabled`);
